...
export default function InvoicesView({ business }: Props) {
  ...
  const handleAddItem = () => {
    ...
    setItems([
      ...items,
      {
        ...
        unitPrice: Number(form.itemPrice)
      }
    ]);
  ...
  /*
   * WHATSAPP
   * Abre WhatsApp Web/App con el mensaje preparado.
   */
...
Te enviamos el comprobante correspondiente a tu compra.

Factura: ${invoice.number}
Tipo: Factura ${invoice.type}
Total: ${formatCurrency(invoice.total)}

Este comprobante es un modelo de prueba y no puede ser utilizado con fines fiscales.
...
${issuer.name || 'Faro'}`;
   ...
   * No depende de mailto ni de la configuración de Windows.
   */
  const handleSendEmail = (invoice: Invoice) => {
    ...
    const subject = `Factura ${invoice.number} - ${issuer.name || 'Faro'
      }`;

    const body = `Hola ${invoice.clientName},
...
${issuer.name || 'Faro'}`;
    ...
    const gmailUrl =
      ...
      `&fs=1` +
      `&to=${encodeURIComponent(invoice.clientEmail)}` +
        ...
        >
          <div>
            ...
            <p className="section-description">
              Generá facturas profesionales y envialas a tus clientes.
            </p>
          </div>
          ...
          <div className="empty-state">
            ...
            <p className="empty-state-desc">
              {search
                ...
                : 'Empezá generando tu primera factura para enviarla a tus clientes.'}
            </p>
                  ...
                  <td data-label="Estado">
                    ...
                    <select
                      ...
                      style={{
                        ...
                        fontWeight: 600,
                        outline: 'none',
                        background: `var(--${statusMap[i.status]?.color ||
                          'gray'
                          }-bg)`,
                        color:
                          statusMap[i.status]?.color ===
                            'gray'
                            ? 'var(--text-secondary)'
                            : `var(--${statusMap[i.status]?.color ||
                            'gray'
                            })`
                      }}
                    >
        ...
        {flowMode === 'select' ? (
          ...
          >
            ...
            >
              <div style={{ textAlign: 'left' }}>
                ...
                >
                  Seleccionar de mi lista de clientes
                </span>
              </div>
            </button>
            ...
            >
              <div style={{ textAlign: 'left' }}>
                ...
                >
                  Ingresar datos manualmente (No se guarda en Clientes)
                </span>
              </div>
            </button>
          ...
          >
            ...
            >
              ...
              <textarea
                className="field-input"
                placeholder="Condiciones de pago, comentarios..."
                value={form.notes}
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
